...
class LayerOptimizer():
    ...
    def opt_params(self):
        assert len(self.layer_groups) == len(self.lrs), 'size mismatch, expected %s lrs, but got %s'.format(len(self.layer_groups), len(self.lrs))
        assert len(self.layer_groups) == len(self.wds), 'size mismatch, expected %s wds, but got %s'.format(len(self.layer_groups), len(self.wds))
...
def zip_strict_(l, r):
    assert len(l) == len(r), 'size mismatch, expected lengths %s, but got %s and %s instead.'.format(len(l), len(l), len(r))
    return zip(l, r)
